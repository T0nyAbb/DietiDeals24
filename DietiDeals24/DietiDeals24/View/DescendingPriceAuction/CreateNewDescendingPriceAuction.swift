//
//  CreateNewDescendingPriceAuction.swift
//  DietiDeals24
//
...
struct CreateNewDescendingPriceAuction: View {
        ...
        if rootIsActive {
            ScrollView(.vertical, showsIndicators: false) {
                ...
                VStack {
                    ...
                    TextField("Starting Price", value: $startingPrice, format: .currency(code: Locale.current.currency?.identifier ?? "€"), prompt: Text("Starting price (default 100 €)"))
                    ...
                    TextField("Starting Price", value: $minimumPrice, format: .currency(code: Locale.current.currency?.identifier ?? "€"), prompt: Text("Minimum selling price*"))
                        ...
                        .onChange(of: minimumPrice) {
                            do {
                                next = try descendingPriceChecker.checkAuctionFields(startingPrice: startingPrice ?? 100, minimumPrice: minimumPrice, decrementAmount: reductionAmount ?? 1, startingDate: selectedDate)
                            } catch {
                    ...
                    Spacer()
                    NavigationLink(destination: DescendingPriceAuctionRecapView(image: uiImage, title: title, description: description, category: category,startingPrice: startingPrice ?? 100, selectedDate: selectedDate, minimumPrice: minimumPrice ?? 0, timerAmount: timerAmount, reductionAmount: reductionAmount ?? 1, popToRoot: self.$rootIsActive)) {
                        HStack {
                            Text("Next")
...
#Preview {
    CreateNewDescendingPriceAuction(rootIsActive: .constant(true))
}
